...
Fix an $\eps$. 
Let us guess the optimum value $D\geq 1$.  We wish to either prove $T$ is too large a guess, or find an assignment which assigns machine $i$ a subset of jobs $S_i$ with {\em load} $\sum_{p\in S_i} c_p \geq D_i := Ds_i$.
