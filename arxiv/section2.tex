...
As a corollary to Theorem~\ref{fthm:weakred}, and using the fact that $Q||C_{min}$ has a PTAS ~\cite{AzarE98},  and our result (Theorem~\ref{thm:q} proved in~\Cref{sec:qptas}) that \cckp has a quasipolynomial time approximation scheme (QPTAS), we get Theorem~\ref{thm:1}.\medskip
