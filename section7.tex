...
%\label{eq:d2}\\
%\textstyle \alpha_j & \geq & 0 \label{eq:d3} % \tag{D3}
%\end{eqnarray}
...
At any point, we have a running iterate $(\alpha,\beta)$. %The non-trivial part is to find a separation oracle for \eqref{eq:d2}.
...
There is a PTAS for this problem~\comment{Cite or Prove}. If the maximum value returned by the approximation scheme is {\em smaller} than $D_i$, then 
...
%satisfies \eqref{eq:d1}-\eqref{eq:d3} and we can exit. 
...
\subsection{Integrality Gap}\label{sec:conf-ig}
