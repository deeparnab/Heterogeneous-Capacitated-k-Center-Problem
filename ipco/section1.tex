...
the {\em machine placement problem}: determine network locations to place a given collection of servers  to minimize the makespan (jobs are located in a metric space, and first need to reach the server they are assigned to before they can be processed)~\cite{PSW97}. %\mcomment{Add Venkat's References?}
...
As mentioned before, our main technical contribution is in showing a connection in the converse direction as well. Indeed, our algorithms for~\Cref{thm:1,thm:2,thm:2a} use the following results we obtain for the $Q|f_i|C_{min}$ and $Q||C_{min}$ problems.
