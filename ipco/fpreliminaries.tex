...
capacity $c_p$ installed on it will be referred to as a {\em type $p$ facility.}
	...
	We force $x_{ijp} = 0$ for all pairs $i,j$ and type $p$ such that  $d(i,j) > \opt$.
		...
		\begin{minipage}{0.45\textwidth}
			\begin{alignat}{4}
				& \quad \forall j\in C,   &&\quad  \textstyle \sum_{i\in F} \sum_{p\in [P]}  x_{ijp} \geq 1 \label{feq:lp1} \tag{\small{L1}}  \\
				& \quad \forall i\in F,p\in [P] ,  &&\quad  \textstyle \sum_{j\in C}  x_{ijp} \leq c_py_{ip} \label{feq:lp2} \tag{\small{L2}} \\
				& \quad \forall p\in [P], && \quad \textstyle \sum_{i\in F} y_{iq}   \leq k_p \label{feq:lp3}  \tag{\small{L3}}
			\end{alignat}
		\end{minipage}
		~\vline~
		\begin{minipage}{0.45\textwidth}
			\begin{alignat}{4}
				& \quad \forall i\in F, j\in C,p\in [P],  && \quad x_{ijp} \leq y_{ip}\label{feq:lp4}   \tag{\small{L4}} \\
				& \quad \forall i\in F, && \quad \textstyle\sum_{p\in [P]} y_{ip} \leq 1 \label{feq:lp5}  \tag{\small{L5}} \\
				& \quad \forall i\in F,j\in C,p\in [P], && \quad x_{ijp},y_{ip} \geq 0\label{feq:lp6}\tag{\small{L6}}
			\end{alignat}
		\end{minipage}
\smallskip
